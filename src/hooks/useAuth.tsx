  ...
  useEffect(() => {
    ...
    const attemptToSaveTokens = async (currentSession: Session | null, eventType: string) => {
      ...
      if (shouldSave) {
        ...
        try {
          ...
          console.log(`AuthProvider (${eventType}) LOG 2: SIMULAÇÃO de saveGoogleTokens BEM SUCEDIDA.`);
        } catch (error) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, currentSession) => {
        ...
        if (event === 'SIGNED_OUT') {
          console.log("AuthProvider onAuthStateChange LOG 5: Evento SIGNED_OUT. Cleaning up and navigating.");
          if (isMounted) tokensSavedForSessionRef.current = null;
          navigate('/');
        }

        if (initialCheckDoneRef.current && loading && isMounted && (event === 'SIGNED_IN' || event === 'SIGNED_OUT' || event === 'USER_UPDATED')) {
    ...
    const checkInitialSession = async () => {
      ...
      if (!isMounted) {
        ...
        if (isMounted && loading) setLoading(false); // Correção aqui
        return;
      }
